...
resource "aws_s3_bucket" "ali-aws" {
  bucket = "ali-aws"
...
resource "aws_s3_bucket_acl" "ali-aws-acl" {
  bucket = aws_s3_bucket.ali-aws.id
  acl    = "private"
}


resource "aws_s3_bucket_versioning" "ali-aws-version" {
  bucket = aws_s3_bucket.ali-aws.id
  versioning_configuration {
    status = "Enabled"
  }
...
resource "aws_codepipeline" "test-pipeline" {
  ...
  artifact_store {
    location = aws_s3_bucket.ali-aws.bucket
    type     = "S3"
  }
  stage {
    name = "Source"
